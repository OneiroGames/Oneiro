  ...
  namespace detail
  {
    void BeginSwapchainRendering(const SwapchainRenderInfo& renderInfo)
    {
      ...
      case AttachmentLoadOp::CLEAR:
      {
        ...
        glClearNamedFramebufferfv(0, GL_COLOR, 0, ri.clearColorValue.data());
        break;
      }
      ...
      for (GLint i = 0; i < static_cast<GLint>(ri.colorAttachments.size()); i++)
      {
        ...
        case AttachmentLoadOp::CLEAR:
        {
          ...
          case detail::GlBaseTypeClass::FLOAT:
            glClearNamedFramebufferfv(context->currentFbo, GL_COLOR, i, ccv.data());
            break;
          case detail::GlBaseTypeClass::SINT:
          {
            std::array<int32_t, 4> clearData{static_cast<int32_t>(ccv[0]),
                                             static_cast<int32_t>(ccv[1]),
                                             static_cast<int32_t>(ccv[2]),
                                             static_cast<int32_t>(ccv[3])};
            glClearNamedFramebufferiv(context->currentFbo, GL_COLOR, i, clearData.data());
            break;
          }
          case detail::GlBaseTypeClass::UINT:
          {
            std::array<uint32_t, 4> clearData{static_cast<uint32_t>(ccv[0]),
                                              static_cast<uint32_t>(ccv[1]),
                                              static_cast<uint32_t>(ccv[2]),
                                              static_cast<uint32_t>(ccv[3])};
      ...
      else
      {
        ...
        for (const auto& attachment : ri.colorAttachments)
        {
          ...
          drawRect.extent.height =
            std::min(drawRect.extent.height, attachment.texture->GetCreateInfo().extent.height);
        }
        if (ri.depthAttachment)
        {
